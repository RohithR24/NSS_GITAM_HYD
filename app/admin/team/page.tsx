...
import {createTeamWithCustomID, fetchAllTeams} from '@/api/index'
...
const AdminDashboard: React.FC = () => {
  ...
  const handleCreateTeam = () => {
    ...
    setNewTeamName(`Team ${new Date().getFullYear()}-${new Date().getFullYear() + 1}`)
  }
  ...
  const handleAddMember = (teamId: number, category: 'faculty' | 'students') => {
  ...
  const handleDeleteMember = (teamId: number, category: 'head' | 'faculty' | 'students', memberId: number) => {
    const updatedTeams = teams.map(team => {
      if (team.id === teamId) {
        if (category === 'head') {
          return { ...team, head: { ...defaultMember, id: Date.now(), role: "NSS Co-Ordinator" } }
        } else {
          return { ...team, [category]: team[category].filter(m => m.id !== memberId) }
        }
      }
      return team
    })
  ...
  const handleSaveMember = (updatedMember: TeamMember) => {
    const updatedTeams = teams.map(team => {
      ...
      return {
        ...team,
        faculty: team.faculty.map(m => m.id === updatedMember.id ? updatedMember : m),
        students: team.students.map(m => m.id === updatedMember.id ? updatedMember : m)
      }
    })
    setTeams(updatedTeams)
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-blue-900 mb-6">NSS Admin Dashboard</h1>
        {!isCreatingTeam ? (
          <button
        ...
        {teams.map(team => (
          <div key={team.id} className="bg-white rounded-lg shadow-md mb-4 overflow-hidden">
            <div
              className="bg-blue-100 p-4 flex justify-between items-center cursor-pointer"
              onClick={() => setExpandedTeam(expandedTeam === team.id ? null : team.id)}
            >
              <h2 className="text-xl font-semibold text-blue-900">{team.name}</h2>
            ...
            {expandedTeam === team.id && (
              <div className="p-4">
                ...
                <div className="flex items-center justify-between bg-gray-50 p-2 rounded-md mb-4">
                  <div className="flex items-center">
                    <img src={team.head.image} alt={team.head.name} className="w-10 h-10 rounded-full mr-2" />
                    <div>
                  ...
                  <div>
                    <button onClick={() => handleEditMember(team.head)} className="text-blue-500 mr-2"><Edit size={18} /></button>
                    <button onClick={() => handleDeleteMember(team.id, 'head', team.head.id)} className="text-red-500"><Trash size={18} /></button>
                  </div>
                </div>
                <h3 className="text-lg font-semibold mb-2">Faculty Members</h3>
                {team.faculty.map(member => (
                  <div key={member.id} className="flex items-center justify-between bg-gray-50 p-2 rounded-md mb-2">
                    <div className="flex items-center">
                      <img src={member.image} alt={member.name} className="w-10 h-10 rounded-full mr-2" />
                      <div>
                    ...
                    <div>
                      <button onClick={() => handleEditMember(member)} className="text-blue-500 mr-2"><Edit size={18} /></button>
                      <button onClick={() => handleDeleteMember(team.id, 'faculty', member.id)} className="text-red-500"><Trash size={18} /></button>
                    </div>
                  </div>
                ))}
                <button
                ...
                {team.students.map(member => (
                  <div key={member.id} className="flex items-center justify-between bg-gray-50 p-2 rounded-md mb-2">
                    ...
                    <div>
                      <button onClick={() => handleEditMember(member)} className="text-blue-500 mr-2"><Edit size={18} /></button>
                      <button onClick={() => handleDeleteMember(team.id, 'students', member.id)} className="text-red-500"><Trash size={18} /></button>
                    </div>
                  </div>
                ))}
                <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-4 rounded-lg w-96">
            ...
            <input
              ...
              onChange={(e) => setEditingMember({ ...editingMember, social: { ...editingMember.social, instagram: e.target.value } })}
              className="w-full p-2 border rounded-md mb-2"
            ...
            <input
              ...
              onChange={(e) => setEditingMember({ ...editingMember, social: { ...editingMember.social, facebook: e.target.value } })}
              className="w-full p-2 border rounded-md mb-4"
              placeholder="Facebook URL"
            />
            <div className="flex justify-end">
              <button onClick={() => setEditingMember(null)} className="bg-gray-300 text-black px-4 py-2 rounded-md mr-2">Cancel</button>
              <button onClick={() => handleSaveMember(editingMember)} className="bg-blue-500 text-white px-4 py-2 rounded-md">Save</button>
            </div>
          </div>
        </div>
      )}
